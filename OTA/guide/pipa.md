# RisingOS Installation Instructions for pipa
 
 **Maintainer:** [Aryan](https://t.me/CuriousNom) | @CuriousNom
 
 ### Required Files
 
 * Download the needed files from [download page](https://risingosrevived.tech/downloads.html?codename=pipa)
 
 * Required device files for first-time flash:
   * boot.img
   * dtbo.img
   * vendor_boot.img
   * ...and then the latest OTA zip for pipa (RisingOS_Revived-x.x.x-xxxxxxxx-CORE-OFFICIAL-pipa-ota.zip)
 ...
 * Use the adb sideload command with the ROM file:
 ...
 ``` 
 adb sideload RisingOS_Revived-x.x.x-xxxxxxxx-CORE-OFFICIAL-pipa-ota.zip
 ```
