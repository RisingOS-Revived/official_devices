...
- Download the following files from the [download page](https://risingosrevived.tech/downloads.html?codename=hotdog)
  * recovery.img
  * boot.img
  * dtbo.img
  * vbmeta.img
  * ...and then the latest OTA zip for hotdog from the [edition](https://sourceforge.net/projects/risingos-revived/files/7.x/) you need

### First Time Installation (clean flash)

- Install the [required drivers](https://wiki.lineageos.org/adb_fastboot_guide) if not done already
- [Unlock the bootloader](https://wiki.lineageos.org/devices/hotdog/install/#unlocking-the-bootloader) if not done already. Make sure to have backed up your data before unlocking bootloader since it will erase all data on your device.
...
``` 
adb sideload RisingOS_Revived-x.x-xxxxxxxx-xxxx-OFFICIAL-hotdog-ota.zip
```
